...
if __name__ == '__main__': 
    ...
    ## extend the size:
    rope_color = hue_detection(img1, poly)
    mask1 = helix_adv_mask(cv2.cvtColor(img1, cv2.COLOR_BGR2HSV)[:,:,0], poly, rope_color)
    mask2 = helix_adv_mask(cv2.cvtColor(img2, cv2.COLOR_BGR2HSV)[:,:,0], poly, rope_color)
